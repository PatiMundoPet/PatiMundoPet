...
const scheduling = template.slice(template.indexOf('<section id="agendamento"'), template.indexOf('<!-- ============================================================ O CAMINHO DO PASSEIO'));
...
assert.doesNotMatch(index + template, /Passeio em Pequeno Grupo|Planos Semanais|Depoimentos|Galeria|Facebook|cartão|boleto|checkout|equipe treinada|famílias atendidas|recomendação/i, 'site público não deve conter conteúdo fictício ou serviços removidos');
assert.match(index + template, /dog walker há nove anos/i, 'Quem é a Pati deve mencionar a experiência profissional real');
assert.match(index, /Escolha uma data para consultar/);
assert.match(index, /A disponibilidade e os horários finais serão confirmados diretamente pelo WhatsApp/);
