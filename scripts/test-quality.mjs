...
assert.equal(count(publicServices, /<h3\b/gi), 2, 'somente dois serviços públicos');
...
assert.deepEqual(scheduling.services, [{ id: 'passeio-individual', label: 'Passeios' }]);
...
assert.equal(count(form, /name="servico"/gi), 1, 'formulário deve oferecer somente Passeios');
