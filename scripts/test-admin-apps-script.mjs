...
function editElement(tag){
  const node={tag,children:[],attributes:{},dataset:{},hidden:false,disabled:false,required:false,value:'',name:'',type:'',textContent:'',className:'',listeners:{},append(...children){this.children.push(...children);for(const child of children){child.parent=this;const form=this.tag==='form'?this:(this.parent&&this.parent.tag==='form'?this.parent:null);if(form&&child.name)form.elements[child.name]=child;if(form){const register=nested=>{if(nested.name)form.elements[nested.name]=nested;for(const deeper of nested.children||[])register(deeper);};register(child);}}},appendChild(child){this.append(child);return child;},replaceChildren(...children){this.children=[];this.append(...children);},setAttribute(name,value){this.attributes[name]=String(value);},addEventListener(name,handler){this.listeners[name]=handler;},focus(){},querySelectorAll(selector){const found=[];const visit=child=>{if(selector==='input'&&child.tag==='input')found.push(child);if(selector==='button'&&child.tag==='button')found.push(child);for(const nested of child.children||[])visit(nested);};for(const child of this.children)visit(child);return found;},reportValidity(){return this.valid!==false;},click(){if(this.listeners.click)this.listeners.click({target:this});if(this.onclick)this.onclick({target:this});if(this.type==='submit'&&!this.disabled){const form=editForms[this.attributes.form];if(form&&form.onsubmit)form.onsubmit({preventDefault(){}});}}};
...
editState.writing=false;editContext.openEditBlock({...timedRow,allDay:true,start:'2026-08-10T00:00:00',end:'2026-08-18T00:00:00',reason:'Férias'},actions.children[0]);editForm=editBody.children[0];assert.equal(editForm.elements.tipo.value,'dia_inteiro');assert.equal(editForm.elements.dataInicial.value,'2026-08-10');assert.equal(editForm.elements.dataFinal.value,'2026-08-17');assert.equal(editForm.elements.motivo.value,'Férias');const beforeGiveUp=editWrites.length;editFoot.children[0].click();assert.equal(editWrites.length,beforeGiveUp,'Desistir não envia edição');
