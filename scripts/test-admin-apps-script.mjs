...
function mockEvent(title,start,end,description='',status='confirmed',allDay=false) { return { title,start,end,description,status,allDay,deleted:false,deleteFails:false,getTitle(){return this.title;},getStartTime(){return this.start;},getEndTime(){return this.end;},getDescription(){return this.description;},getEventStatus(){return this.status;},getId(){return this.id;},isAllDayEvent(){return this.allDay;},deleteEvent(){if(this.deleteFails)throw Error('delete');this.deleted=true;},setTime(start,end){if(this.failSetTime-->0)throw Error('setTime');this.start=start;this.end=end;this.allDay=false;return this;},setAllDayDates(start,end){if(this.failSetAllDay-->0)throw Error('setAllDay');this.start=start;this.end=end;this.allDay=true;return this;},setTitle(title){if(this.failSetTitle-->0)throw Error('setTitle');this.title=title;return this;},setDescription(description){if(this.failSetDescription-->0)throw Error('setDescription');this.description=description;return this;} }; }
...
assert.equal(safeCode(()=>adminContext.editarBloqueio(edited.id,blockId,changed)),'');assert.equal(edited.start.toISOString().slice(11,16),'12:22','repetição idempotente não altera o período');
...
const staleState={writing:false,refreshRequired:true};let blockedCalls={criarBloqueio:0,excluirBloqueio:0,solicitacao:0};for(const operation of Object.keys(blockedCalls)){if(uiContext.tryStartWrite_(staleState))blockedCalls[operation]++;}assert.deepEqual(blockedCalls,{criarBloqueio:0,excluirBloqueio:0,solicitacao:0},'recuperação bloqueia criação, exclusão e ações de Solicitações');
