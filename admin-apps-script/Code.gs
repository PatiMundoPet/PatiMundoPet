...
// Exceção deliberada e auditável: confirma mesmo que o horário já tenha outro atendimento
// CONFIRMADO, exclusivamente para Passeios (validado a partir da própria linha da planilha, não
// do que o cliente manda) — pensada pra passeio em grupo, quando a Pati decide conscientemente
// atender mais de um cliente/pet no mesmo horário. Nunca ignora um bloqueio de disponibilidade.
function confirmarSolicitacaoEmGrupo(requestId, observacao) { return safelyWrite_(requestId, observacao, true, decisionWriter_(function (context) { context.allowGroupOverlap = true; return confirmRequest_(context); })); }
...
function confirmRequest_(context) {
  ...
  var snapshot = context.values.slice(), resolution = resolveConfirmationClient_(context); resolution.config = context.config; var plan = prepareConfirmationClient_(context, resolution), interval = interval_(context);
  // A exceção de grupo (confirmarSolicitacaoEmGrupo) nunca ignora um bloqueio de disponibilidade
  // — só o conflito contra outros atendimentos já confirmados, e só para Passeios. O serviço vem
  // sempre da própria linha da planilha, nunca de nada enviado pelo cliente/interface.
  var requestServiceId = text_(context.record['serviço']), allowGroupOverlap = Boolean(context.allowGroupOverlap) && requestServiceId === 'passeio-individual';
  if (hasConflict_(context.availability, interval) || (!allowGroupOverlap && hasConflict_(context.appointments, interval))) throw safeError_('INTERVAL_UNAVAILABLE', 'O período solicitado não está disponível.');
  ...
  var event, serviceLabel = serviceLabel_(requestServiceId), description = marker_(context.requestId) + '\nserviço: ' + serviceLabel + '\nserviceId: ' + requestServiceId + '\nresponsável: ' + text_(context.record['responsável']) + '\npet: ' + text_(context.record.pet) + '\nWhatsApp: ' + text_(context.record.WhatsApp) + '\ne-mail: ' + text_(context.record['e-mail']) + '\nregião: ' + text_(context.record['região']) + '\nobservações: ' + text_(context.record['observações']) + (allowGroupOverlap ? '\ngrupo: confirmado sobre horário já ocupado (decisão manual)' : '');
