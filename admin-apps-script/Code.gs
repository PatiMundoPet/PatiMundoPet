...
function normalizeClientEmail_(value) { var email = String(value || '').trim(); if (!email) return ''; if (email.length > 254 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw safeError_('INVALID_CLIENT', 'Informe um e-mail válido.'); return email; }
...
function setClientFormFields_(row, headers, input) { row[headers.indexOf('responsável')] = input.responsible; row[headers.indexOf('WhatsApp')] = input.whatsapp; row[headers.indexOf('e-mail')] = input.email; row[headers.indexOf('pets')] = input.pets.join(', '); row[headers.indexOf('observações')] = input.notes; }
