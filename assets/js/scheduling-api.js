(function (root, factory) {
  ...
  function normalizeResponse(value, kind) {
    ...
    if (kind === 'availability' && value.ok) {
      if (value.code !== 'AVAILABILITY_OK' || !value.data || !/^\d{4}-\d{2}-\d{2}$/.test(value.data.date) ||
          !Array.isArray(value.data.available) || !Array.isArray(value.data.unavailable) ||
          value.data.available.concat(value.data.unavailable).some(function (time) { return typeof time !== 'string' || !/^(?:[01]\d|2[0-3]):[0-5]\d$/.test(time); })) throw new Error('INVALID_RESPONSE');
    }
  ...
  function createClient(config, dependencies) {
    ...
    async function call(url, options, kind) {
      ...
      try {
        ...
        return normalizeResponse(json, kind);
      } catch (error) {
    ...
    function get(action, date, kind) {
      ...
      return call(url.toString(), { method: 'GET', cache: 'no-store', redirect: 'follow' }, kind);
    }
    ...
    return {
      ...
      availability: function (date) {
        ...
        return get('availability', date, 'availability');
      },
